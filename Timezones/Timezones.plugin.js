...
.timezone-badge {
    ...
    color: var(--text-normal);
                  ...
                  if (!TextInput) {
                      console.error("Failed to load TextInput module");
                      return;
                  }

                  const Markdown = Webpack.getModule((m) => m.default?.rules && m.default?.defaultProps?.parser).default;
                  if (!Markdown) {
                      console.error("Failed to load Markdown module");
                      return;
                  }

                  const SearchableSelect = Webpack.getByKeys("Button", "SearchableSelect")?.SearchableSelect;
                  if (!SearchableSelect) {
                      console.error("Failed to load SearchableSelect module");
                      return;
                  }

                  const i18n = Webpack.getByKeys("getLocale");
                  if (!i18n) {
                      console.error("Failed to load i18n module");
                      return;
                  }
                  ...
                  return class Timezones extends Plugin {
                      async onStart() {
                          ...
                          if (!ProfileBanner) {
                              console.error("Failed to load ProfileBanner module");
                              return;
                          }

                          const MessageHeader = Webpack.getModule((m) => m.default?.toString().includes(".getMessageTimestampId)("));
                          if (!MessageHeader) {
                              console.error("Failed to load MessageHeader module");
                              return;
                          }

                          const Tooltip = Components.Tooltip;
                          if (!Tooltip) {
                              console.error("Failed to load Tooltip component");
                              return;
                          }
